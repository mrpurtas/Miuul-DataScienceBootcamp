...
df.columns
resultt = ["NUM_" + x.upper() if df else x.upper() for x in df.columns]
print(resultt)
...
resultt = ["NUM_" + col.upper() if pd.api.types.is_numeric_dtype(df[col]) else col.upper() for col in df.columns]
print(resultt)
...
def alternate(string):
    new_str = ""
    for index_info in range(len(string)):
        if index_info % 2 == 0:
            new_str += string[index_info].upper()
        else:
            new_str += string[index_info].lower()
    print(new_str)


alternate("merhabanaber")