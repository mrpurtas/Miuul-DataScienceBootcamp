...
"""df["CreateDate"].dt.strftime("%Y-%m") kodu, CreateDate sütunundaki datetime türündeki değerleri belirtilen formatla yeniden biçimlendirir.
 Bu özel biçimlendirme, tarihleri yıl ve ay (YYYY-MM) formatında döndürür.

df["NEW_DATE"] = ... ifadesi ile de bu biçimlendirilmiş tarih değerleri yeni bir sütun olan NEW_DATE sütununa atanır.

Özetle, bu kod, CreateDate sütunundaki tarihleri YYYY-MM formatında alır ve bu formatlanmış tarihleri yeni bir sütun
 olan NEW_DATE sütununa ekler. Örneğin, eğer CreateDate sütununda "2023-10-26" tarihi varsa, NEW_DATE sütununda bu değer için "2023-10" şeklinde bir kayıt oluşacaktır."""
...
product_df = df.groupby(["ID", "hizmet"])["hizmet"].count().unstack().fillna(0).applymap(lambda x: 1 if x > 0 else 0)
# Adım 2: Birliktelik kurallarını oluşturunuz.
frequent_itemsets = apriori(product_df,
                            min_support=0.01,
                            use_colnames=True)
